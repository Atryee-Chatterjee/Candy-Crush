...
function touchEnd(e) {
    ...
    // Minimum swipe distance to register as a move
    let minSwipeDistance = 30;

    if (Math.max(absDeltaX, absDeltaY) < minSwipeDistance) {
        // Touch was too short, treat as a tap (maybe for selection)
        selectedTile = null;
        return;
    }

    // Get current tile coordinates
    let currCoords = selectedTile.id.split("-");
    let r = parseInt(currCoords[0]);
    let c = parseInt(currCoords[1]);
    ...
    if (targetR >= 0 && targetR < rows && targetC >= 0 && targetC < columns) {
        ...
        if (!selectedTile.src.includes("blank") && !targetTile.src.includes("blank")) {
            // Swap the candies
            let tempSrc = selectedTile.src;
            selectedTile.src = targetTile.src;
